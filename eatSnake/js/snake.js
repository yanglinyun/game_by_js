window.onload = function(){
	...
	var Snake = function(row,col,snakeId,speed,isAuto){
		this.row = row || 20 ;
		this.col = col || 20 ;	//创建地图的行跟列数
	...
	Snake.prototype = {
		...
		randomPoint:function(initX,initY,endX,endY){	
			var initx = initX||0;
			var inity = initY||0;
			var endx = endX||(this.col-1);
			var endy = endY||(this.row-1);
			var p=[];
		...
		createGrid:function(){	
			...
			for(var i=0;i<this.row;i++){
				...
				for(var j=0;j<this.col;j++){
					...
					td.style.height=bodyH * 0.8 / this.row + 'px'
					td.style.width=bodyW / this.col + 'px'
				}
			...
			var tableH  = document.getElementsByTagName('table')[0].offsetHeight
			document.getElementsByClassName('controller')[0].style.height = (bodyH - tableH) + 'px';
		},
		//设置蛇的背景色
		painSnake:function(){
			var snakeGrid = this.snakeGrid ;
		...
		controller:function(){
			...
			if(head_x<food_x){ 		//食物在蛇的下方
				if(!this.pointInSnake([head_x+1,head_y])&&this.derectkey!=38&&(head_x+1)<this.col){
					this.derectkey = 40;
				}
				else if(!this.pointInSnake([head_x,head_y+1])&&this.derectkey!=37&&(head_y+1)<this.row){
					this.derectkey = 39;
				}
				...
				else if(!this.pointInSnake([head_x+1,head_y])&&this.derectkey!=38&&(head_x+1)<this.col){
					this.derectkey = 40;
				}
				else{
					this.derectkey = 37;
				...
				if(!this.pointInSnake([head_x,head_y+1])&&this.derectkey!=37&&(head_y+1)<this.row){
					this.derectkey = 39;
				}
				else if(!this.pointInSnake([head_x+1,head_y])&&this.derectkey!=38&&(head_x+1)<this.col){
					this.derectkey = 40;
		...
		main:function(){
			...
			if(headx==this.foodGrid[0]&&heady==this.foodGrid[1]){
				...
				//判断是否撞墙
					...
					var point = [this.snakeGrid[0][0],this.snakeGrid[0][1]]
					if(point[0]<0||point[0]>this.col-1){
						this.snakeGrid[0][0] = point[0]<0?this.col-1:0;
					}else if(point[1]<0||point[1]>this.row-1){
						this.snakeGrid[0][1] = point[1]<0?this.row-1:0
					}
				}
				//判断游戏是否结束
				if(isEnd){
		...
		init:function(){
			...
			_this.Grid = _this.multiArray(_this.row, _this.col);
